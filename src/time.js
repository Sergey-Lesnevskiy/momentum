...
const dateSetting = setting.querySelector(".date_setting");
const timeSetting = setting.querySelector(".time_setting");
const weatherSetting = setting.querySelector(".weather_setting");
const qoeteSetting = setting.querySelector(".qoute_setting");
...
dateSetting.addEventListener('click',()=>{
  pageDate.classList.toggle('dis_active')
  dateSetting.classList.toggle('active')
})
timeSetting.addEventListener('click',()=>{
  time.classList.toggle('dis_active')
  timeSetting.classList.toggle('active')
})
qoeteSetting.addEventListener('click',()=>{
  footer.classList.toggle('dis_active')
  qoeteSetting.classList.toggle('active')
})
...
function getRandomNum() {
  randomNum = Math.floor(Math.random() * testArraw.length) + 1;
}
getRandomNum();
...
function getSlideNext() {
  ...
  getLinkToImage2() 
  // showGreeting(isLangMeet,isLanguage);
}

function getSlidePrev() {
 randomNum <= 1 ? (randomNum = testArraw.length) : randomNum--;
 getLinkToImage2() 
  // showGreeting(isLangMeet,isLanguage);
}
...
function setBg(data) {
  ...
  // let bgNum = randomNum;
  // bgNum >= 10 ? bgNum : (bgNum = String(bgNum).padStart(2, "0"));
// console.log(data.photos.photo[randomNum].url_l)
  ...
  for (let i = 0; i < data.photos.photo.length; i++) {
    if(data.photos.photo[i].url_l===undefined){
      i++
    }else{testArraw.push(data.photos.photo[i].url_l)}
...
//slider
//Greeting
function getTimeOfDay(cange='en') {
